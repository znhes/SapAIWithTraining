...
createApp({
    ...
    methods: {
        ...
        async loadStats() {
            try {
                ...
                this.stats = {
                    ...
                    aiStatus: ollamaStatus.status === 'running' && ollamaStatus.deepseek_available
                };
            } catch (error) {
        ...
        async generateTrainingData() {
            ...
            this.trainingStatus.message = 'Generating training data from knowledge base...';
            ...
            try {
                ...
                if (!response.ok) {
                    const errorData = await response.json();
                    throw new Error(errorData.error || 'Failed to generate training data');
                }
        ...
        async trainModel() {
            this.trainingStatus.loading = true;
            this.trainingStatus.message = 'Training custom AI model... This may take several minutes.';
        ...
        async exportTrainingData() {
            try {
                ...
                this.showAlert('Error exporting training data: ' + error.message, false);
            }
        },
...
async testModels() {
    try {
        ...
        Object.entries(data.test_results).forEach(([model, result]) => {
            message += `\n${model}: ${result.working ? '✅ Working' : '❌ Failed'}\n`;
